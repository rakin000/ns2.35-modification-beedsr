...
struct route_reply
{
	...
	int rep_rtlen_ = 0 ; /* # hops in route reply */
...
public:
	...
	inline void route_reply_add_addrs(double node_energy, double node_pos_x,double node_pos_y,double node_pos_z){
		assert (sr_reply_.rep_rtlen_ < MAX_SR_LEN ) ;
		// if( sr_replly_.rep_rtlen_ < MAX_SR_LEN ){
			sr_reply_.rep_addrs_[sr_reply_.rep_rtlen_].node_energy = node_energy ;
			sr_reply_.rep_addrs_[sr_reply_.rep_rtlen_].pos_x = node_pos_x ;
			sr_reply_.rep_addrs_[sr_reply_.rep_rtlen_].pos_y = node_pos_y ;
