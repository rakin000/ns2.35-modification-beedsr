...
static const int default_flow_timeout = 5; //10;
...
void DSRAgent::handleDefaultForwarding(SRPacket &p)
{
...
void DSRAgent::handleFlowForwarding(SRPacket &p, int flowidx)
{
  hdr_sr *srh = hdr_sr::access(p.pkt);
  hdr_ip *iph = hdr_ip::access(p.pkt);
...
void DSRAgent::handleForwarding(SRPacket &p)
...
void DSRAgent::handleRouteRequest(SRPacket &p)
/* process a route request that isn't targeted at us */
{
  ...
  if ((srh->max_propagation() == 0 || dsragent_reply_from_cache_on_propagating) && replyFromRouteCache(p))
    return; // all done
...
/*===========================================================================
  Helpers
---------------------------------------------------------------------------*/
bool DSRAgent::ignoreRouteRequestp(SRPacket &p)
...
void DSRAgent::getRouteForPacket(SRPacket &p, bool retry)
...
int DSRAgent::diff_subnet(ID dest, ID myid)
{
  int dst = dest.addr;
  int id = myid.addr;
...
void DSRAgent::processUnknownFlowError(SRPacket &p, bool asDefault)
{
  hdr_sr *srh = hdr_sr::access(p.pkt);
  int flowidx = -1;
...
void DSRAgent::processBrokenRouteError(SRPacket &p)
...
void DSRAgent::processFlowARS(const Packet *packet)
{
...
void DSRAgent::sendRouteShortening(SRPacket &p, int heard_at, int xmit_at)
...
// tell them this.
{
  ...
  else if (Scheduler::instance().clock() - g->t < grat_hold_down_time)
    return;
...
void DSRAgent::undeliverablePkt(Packet *pkt, int mine)
...
void DSRAgent::sendUnknownFlow(SRPacket &p, bool asDefault, u_int16_t flowid)
{
  hdr_sr *srh = hdr_sr::access(p.pkt);
  hdr_ip *iph = hdr_ip::access(p.pkt);
  hdr_cmn *cmh = hdr_cmn::access(p.pkt);
...
void DSRAgent::xmitFlowFailed(Packet *pkt, const char *reason)
{
  hdr_sr *srh = hdr_sr::access(pkt);
  hdr_ip *iph = hdr_ip::access(pkt);
...
inline void DSRAgent::updateNodeInformationToNetID()
{
  if (node_)
  {
...
void DSRAgent::appendNodeInformationToReplyHeader(hdr_sr *srh)
{
