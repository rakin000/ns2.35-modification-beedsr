...
int MobiCache::command(int argc, const char *const *argv)
{
...
void MobiCache::periodic_checkCache()
{
  int c;
  int route_count = 0;
  ...
  for (c = 0; c < primary_cache->size; c++)
  {
    ...
    if (primary_cache->cache[c].length() == 0)
      continue;
  ...
  for (c = 0; c < secondary_cache->size; c++)
  {
    ...
    if (secondary_cache->cache[c].length() == 0)
      continue;
...
void MobiCache::noticeDeadLink(const ID &from, const ID &to, Time)
// the link from->to isn't working anymore, purge routes containing
// it from the cache
{
  if (verbose_debug)
...
bool MobiCache::findRoute(ID dest, Path &route, int for_me)
// if there is a cached path from us to dest returns true and fills in
// the route accordingly. returns false otherwise
...
bool Cache::searchRoute(const ID &dest, int &i, Path &path, int &index)
// look for dest in cache, starting at index,
...
void Cache::noticeDeadLink(const ID &from, const ID &to)
// the link from->to isn't working anymore, purge routes containing
// it from the cache
{
...
int Cache::pickVictim(int exclude)
// returns the index of a suitable victim in the cache
 ...
 */
void MobiCache::checkRoute(Path &p,
...
void MobiCache::checkRoute(Path *p, int action, int prefix_len)
{
  int c;
  int subroute_bad_count = 0;
  int tested = 0;
