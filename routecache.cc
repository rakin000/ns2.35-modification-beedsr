...
int RouteCache::command(int argc, const char *const *argv)
{
  if (argc == 2)
  {
...
int RouteCache::pre_addRoute(const Path &route, Path &rt,
                             Time t, const ID &who_from)
{
  ...
  for (int c = 0; c < rt.length(); c++)
  {
    ...
    if (rt[c] == who_from)
      kind = LT_UNTESTED; // remaining ids came from $
    rt[c].link_type = kind;
    rt[c].t = t;
  }
...
int RouteCache::pre_noticeRouteUsed(const Path &p, Path &stub,
                                    Time t, const ID &who_from)
{
  int c;
  ...
  { // we are in the path, extract the subpath
...
void MobiHandler::handle(Event *)
{
  cache->periodic_checkCache();
  Scheduler::instance().schedule(this, &intr, interval);
}

int RouteCache::checkRoute_logall(Path *p, int action, int start)
{
  int c;
